...
// Add these requires at the top of your file after session_start()
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\Exception;

require 'vendor/autoload.php'; // If using composer
// OR require these if manually downloaded
...
// Replace your existing sendManagerCredentials function with this:
function sendManagerCredentials($email, $password) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'elizabethmaryabraham09@gmail.com'; // Replace with your email
        $mail->Password   = 'xvec mfoh vkhp fabg'; // Replace with your password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;

        //Recipients
        $mail->setFrom('elizabethmaryabraham09@gmail.com', 'Admin Team');
        $mail->addAddress($email);

        //Content
        $mail->isHTML(true);
        $mail->Subject = 'Your Manager Account Credentials';
        $mail->Body    = "
            <h2>Welcome to Our Team!</h2>
            <p>Your manager account has been created. Here are your credentials:</p>
        ...
        $mail->AltBody = "
            ...
            Your manager account has been created. Here are your credentials:
...
// Modify the form processing logic for adding a manager
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_manager'])) {
    ...
    try {
        ...
        $sql = "INSERT INTO users (name, email, dob, mobile, password, role) 
                VALUES (:name, :email, :dob, :mobile, :password, 'manager')"; // Changed from 2 to 'manager'
        ...
        // Store credentials in session for testing (temporary solution)
        $_SESSION['temp_credentials'] = [
            'email' => $email,
            'password' => $password
        ];
        $success_message = "Manager added successfully! Check the credentials below.";
        $conn->commit();
...
function getManagers($conn) {
    $sql = "SELECT user_id, name, email, mobile, dob FROM users WHERE role = 'manager' ORDER BY user_id DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<body>
    <aside class="sidebar">
        ...
        <nav>
            <ul class="sidebar-nav">
                ...
                <li class="sidebar-nav-item active"><a href="admin_manager.php">Manager</a></li>
    ...
    <main class="dashboard-content">
        <div class="section-content">
            ...
            <form method="POST" action="" class="add-manager-form">
        ...
        <div class="manager-list">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td><?php echo htmlspecialchars($manager['user_id']); ?></td>
                        ...
                        <td class="action-buttons">
                            <button class="btn btn-edit" onclick="editManager(<?php echo $manager['user_id']; ?>)">Edit</button>
                            <button class="btn btn-danger" onclick="deleteManager(<?php echo $manager['user_id']; ?>)">Delete</button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        function editManager(userId) {
            ...
            alert('Edit manager with ID: ' + userId);
        }

        function deleteManager(userId) {
            if (confirm('Are you sure you want to delete this manager?')) {
                // Add your delete functionality here
                window.location.href = `admin_manager.php?delete_manager=${userId}`;
            }
        }
        </script>
        ...
        <?php if (isset($_SESSION['temp_credentials'])): ?>
            <div class="alert alert-info">
                <h4>Testing Only - New Manager Credentials:</h4>
                <p>Email: <?php echo htmlspecialchars($_SESSION['temp_credentials']['email']); ?></p>
                <p>Password: <?php echo htmlspecialchars($_SESSION['temp_credentials']['password']); ?></p>
                <?php unset($_SESSION['temp_credentials']); // Clear after displaying ?>
            </div>
        <?php endif; ?>
    </main>
</body>
...
// Update the delete manager query
if (isset($_GET['delete_manager'])) {
    $manager_id = $_GET['delete_manager'];
    try {
        $sql = "DELETE FROM users WHERE user_id = :id AND role = 'manager'";
        $stmt = $conn->prepare($sql);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $error_message = "Manager not found or could not be deleted";
        }
        ...
        $error_message = "Error deleting manager: " . $e->getMessage();
    }
}